function [O ,O2 ,O3 ,O4 , DzDw1 , DzDw2, DzDw3 ] = xfinal(z, beta, t, p, y, gamma, Rho, Epsilon, DzDy)
%% network setting
config;
...
if nargin == 9
    ...
    for k = 1:fN
        ...
        O(:,:,k) = Rho(k)*S; % dxdz,为下一次计算Z的参数(dEdq)做准备
        O2(:,:,k) = (-1)*Rho(k)*S; % dxd(beta),为下一次计算M的参数(dEd(eta))做准备
    end        
        O3 = Trans * Denom1;%dx/dt
        O4 = (-1)*Trans * Denom1;%dx/dp
    % DzDw1
    A = (-1)* Q .* Q;
    Ds=zeros(m,n);
    for i=1:fN
    ...
    for i=1:fN
        for j=1:s
            ...
            PS1=psf2otf(H(:,:,i), [m,n]);
            PS3=2*PS.*PS1;
            tl=real(DzDy.*ifft2(Rho(i)*A.*PS3.*Njj));%第一项
            tp1=z(:,:,i)-beta(:,:,i);
            DS = imfilter(double(tp1) ,double(Bj1),'same','circular','conv');
            tr = real(DzDy.*ifft2(Rho(i)*Q.*fft2(DS)));%第二项
            tw1 = tl +tr;
    ...
    for k=1:fN
        ...
        tp4 = fft2(tp4);
        temp1 = real(DzDy.*ifft2(A.*Nii.*Njj ));% 第一项
        temp2 = real(DzDy.*ifft2( Q.*tp4));
        tw2 = temp1 + temp2;
        DzDw2(k) = sum(tw2(:)); % dEd(rho)
    end
    tt = real(DzDy.*ifft2(Q.*fft2( Pi)));   
